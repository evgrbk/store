...
class PayeerPaymentService extends PaymentService
{
    protected $provider = 'payeer';
    ...
    protected function getUrl(Payment $payment): array
    {
        try {
            ...
            $key = $this->getSetting('shop_id');

            if (!$key) {
                throw new \Exception('Оплата Payeer временно не доступна.');
            }
